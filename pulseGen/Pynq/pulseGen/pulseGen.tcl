...
################################################################
# This is a generated script based on design: pulseGen_bd
#
...
# Hierarchical cell: PS_interconnect
proc create_hier_cell_PS_interconnect { parentCell nameHier } {

  variable script_folder

  if { $parentCell eq "" || $nameHier eq "" } {
     catch {common::send_msg_id "BD_TCL-102" "ERROR" "create_hier_cell_PS_interconnect() - Empty argument(s)!"}
     return
  }
  ...
  connect_bd_intf_net -intf_net Conn1 [get_bd_intf_pins S00_AXI] [get_bd_intf_pins axi_smc/S00_AXI]
  connect_bd_intf_net -intf_net Conn2 [get_bd_intf_pins M01_AXI] [get_bd_intf_pins ps7_0_axi_periph/M01_AXI]
...
proc create_root_design { parentCell } {
  ...
  set pulse [ create_bd_port -dir O -from 7 -to 0 -type data pulse ]
  set timestamp [ create_bd_port -dir O -from 7 -to 0 -type data timestamp ]
  set trig [ create_bd_port -dir O -from 0 -to 0 trig ]

  # Create instance: PS_interconnect
  create_hier_cell_PS_interconnect [current_bd_instance .] PS_interconnect
  ...
  set block_cell_name pulseGen_0
  if { [catch {set pulseGen_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_msg_id "BD_TCL-105" "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $pulseGen_0 eq "" } {
     catch {common::send_msg_id "BD_TCL-106" "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
    set_property -dict [ list \
  ...
  # Create instance: xlslice_idle, and set properties
  set xlslice_idle [ create_bd_cell -type ip -vlnv xilinx.com:ip:xlslice:1.0 xlslice_idle ]
  set_property -dict [ list \
   CONFIG.DIN_FROM {0} \
   CONFIG.DIN_TO {0} \
   CONFIG.DIN_WIDTH {3} \
  ...
  # Create instance: xlslice_run, and set properties
  set xlslice_run [ create_bd_cell -type ip -vlnv xilinx.com:ip:xlslice:1.0 xlslice_run ]
  set_property -dict [ list \
   CONFIG.DIN_FROM {1} \
   CONFIG.DIN_TO {1} \
   CONFIG.DIN_WIDTH {3} \
  ...
  # Create instance: xlslice_trig, and set properties
  set xlslice_trig [ create_bd_cell -type ip -vlnv xilinx.com:ip:xlslice:1.0 xlslice_trig ]
  set_property -dict [ list \
   CONFIG.DIN_FROM {1} \
   CONFIG.DIN_TO {1} \
   CONFIG.DIN_WIDTH {26} \
  ...
  # Create interface connections
  connect_bd_intf_net -intf_net PS_interconnect_M01_AXI [get_bd_intf_pins PS_interconnect/M01_AXI] [get_bd_intf_pins axi_dma_0/S_AXI_LITE]
  connect_bd_intf_net -intf_net axi_dma_0_M_AXIS_MM2S [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S] [get_bd_intf_pins pulseGen_0/streamDown]
  connect_bd_intf_net -intf_net axi_dma_0_M_AXI_MM2S [get_bd_intf_pins PS_interconnect/S00_AXI] [get_bd_intf_pins axi_dma_0/M_AXI_MM2S]
  connect_bd_intf_net -intf_net processing_system7_0_DDR [get_bd_intf_ports DDR] [get_bd_intf_pins PS_interconnect/DDR]
  connect_bd_intf_net -intf_net processing_system7_0_FIXED_IO [get_bd_intf_ports FIXED_IO] [get_bd_intf_pins PS_interconnect/FIXED_IO]
  ...
  connect_bd_net -net xlslice_trig_Dout [get_bd_ports trig] [get_bd_pins pulseGen_0/trig] [get_bd_pins xlslice_trig/Dout]

  # Create address segments
  create_bd_addr_seg -range 0x20000000 -offset 0x00000000 [get_bd_addr_spaces axi_dma_0/Data_MM2S] [get_bd_addr_segs PS_interconnect/processing_system7_0/S_AXI_HP0/HP0_DDR_LOWOCM] SEG_processing_system7_0_HP0_DDR_LOWOCM
  create_bd_addr_seg -range 0x00010000 -offset 0x40400000 [get_bd_addr_spaces PS_interconnect/processing_system7_0/Data] [get_bd_addr_segs axi_dma_0/S_AXI_LITE/Reg] SEG_axi_dma_0_Reg
  create_bd_addr_seg -range 0x00010000 -offset 0x41200000 [get_bd_addr_spaces PS_interconnect/processing_system7_0/Data] [get_bd_addr_segs axi_gpio_0/S_AXI/Reg] SEG_axi_gpio_0_Reg
